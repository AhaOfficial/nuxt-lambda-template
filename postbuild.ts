...
export default () => {
  ...
  const find = staticPath => {
    ...
    for (const folder of collect(staticPath))
    ...
    for (const folderName of folders) {
      ...
      try {
        ...
        const packageFileData = JSON.parse(
          String(fs.readFileSync(packageFilePath))
        )
        if (packageFileData.name !== undefined) {
          let isIgnored = false
          for (const ignoreRegex of doesntNeedTranspile) {
            if (new RegExp(ignoreRegex).test(packageFileData.name)) {
              isIgnored = true
              break
            }
          }
          if (!isIgnored) moduleNames.push(packageFileData.name)
        }

        const originDependencies = [
          ...Object.keys(packageFileData.dependencies)
        ]
        const fixedDependencies = []
        let isIgnored = false
        for (const originDependency of originDependencies) {
          for (const ignoreRegex of doesntNeedTranspile) {
            if (new RegExp(ignoreRegex).test(originDependency)) {
              isIgnored = true
              break
            }
          }
          if (!isIgnored) fixedDependencies.push(originDependency)
        }
        moduleNames = [...moduleNames, ...fixedDependencies]
      } catch (e) {}
    }
    return moduleNames
  }

  let babelModuleTargets = []
