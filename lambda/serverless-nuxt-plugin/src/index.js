...
class ServerlessNuxtPlugin {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      'after:aws:deploy:deploy:uploadArtifacts': this.afterUploadArtifacts.bind(
        this
      ),
  ...
  async build() {
    ...
    this.serverless.cli.consoleLog(
      `Serverless Nuxt Plugin: ${chalk.yellow('build nuxt')}`
    )
    ...
    const assetBasePath = config.cdnPath
      ? config.cdnPath.replace(/\/+$/, '')
    ...
    await nuxt.ready() // * 유일하게 바꾼 부분이 여기입니다.
    ...
    console.log('👀 ServerlessNuxtPlugin build end')
  }

  async afterUploadArtifacts() {
    this.serverless.cli.consoleLog(
      `Serverless Nuxt Plugin: ${chalk.yellow('after upload artifacts')}`
    )
  ...
  async afterUpdateStack() {
    this.serverless.cli.consoleLog(
      `Serverless Nuxt Plugin: ${chalk.yellow('after update stack')}`
    )
  ...
  async upload(ignoreBucketExists = false) {
    ...
    try {
      ...
      if (ignoreBucketExists) {
        this.serverless.cli.consoleLog(
          `Serverless Nuxt Plugin: ${chalk.yellow('not found asset bucket')}`
        )
        return
      }
    ...
    this.serverless.cli.consoleLog(
      `Serverless Nuxt Plugin: ${chalk.yellow('upload asset files')}`
    )
    const assetsFiles = await globby(assetsPath, { onlyFiles: true })
    await Promise.all(
      assetsFiles.map(file => {
        const fileTargetPath = [config.version, file.replace(assetsPath, '')]
          .join('/')
          .replace(/^\/+|\/+$/, '')
          .replace(/\/+/, '/')
        ...
        return s3
          .putObject({
            ...
            CacheControl: `public, max-age=${Math.floor(
              ms(config.assetsCacheMaxAge) / 1000
            )}` // let the browser cache all static files for 1 year since they receive a unique hash by Nuxt
          })
          .promise()
      })
    )
